...
{
	model = std::make_shared<Stage3Model>();
}

Stage3Controller::Stage3Controller(std::shared_ptr<Stage3Model> model)
{
	this->model = model;
}

void Stage3Controller::registerSelf()
{
	EventManager::getInstance().addObserver(shared_from_this());
}

void Stage3Controller::update(std::shared_ptr<Event> event)
{
...
void Stage3Controller::updateCamera()
{
	...
	Vector3 newPosition = Vector3Add(cameraOffset, model->getPlayerData()->getPlayerPos());
	//if (newPosition.x > -60)
	//{
	...
	//camera.position = { -80, 25, 0 };
	//camera.target = {0.0f, 0.0f, 0.0f};
	//std::cout << "ttttttttttttttttttttttttttttttttttt" << model->getCameraPos() <<'\n';
