...
Stage1Model::Stage1Model() : m_width(0), m_depth(0), m_height(0) {}

Stage1Model::Stage1Model(int width, int height, int depth, btDiscreteDynamicsWorld* dynamicsWorld)
    ...
    auto createLeftRouletteBlocks = [&]() {
        for (int i = 126; i < 130; ++i)
        {
            for (int j = 2; j <= i - 124; ++j)
            {
                ...
                {
                    Vector3 position = { size * i * scale.x, size * j * scale.y, size * k * scale.z };
                    auto rouletteBlock = BlockFactory::createBlock(BlockType::RouletteBlock, dynamicsWorld, PATH_ROULETTEBLOCK, position, scale, rotationAxis, rotaionAngle);
    ...
    auto createRightRouletteBlocks = [&]() {
        ...
        for (int i = 131; i < 135; ++i)
        {
            for (int j = 2; j <= 136 - i; ++j)
            {
                ...
                {
                    Vector3 position = { size * i * scale.x, size * j * scale.y, size * k * scale.z };
                    auto rouletteBlock = BlockFactory::createBlock(BlockType::RouletteBlock, dynamicsWorld, PATH_ROULETTEBLOCK, position, scale, rotationAxis, rotaionAngle2);
                    m_map.push_back(std::shared_ptr<BlockData>(rouletteBlock));
                }
            }
        }
